...
| 11 | **Tests for the riskiest surfaces:** `_handle_ev` solar state machine, `verify_and_retry_pending_commands`, `is_boiler_full`, `PowerBuffer` | §5.2, §5.5 | ⚠️ partial — `verify_and_retry` covered (7 tests `2c27fe6` + 4 more `70825e5`). `_handle_ev` solar covered (Story 1.4, ~22 tests in `test_ev_solar_state_machine.py`). `is_boiler_full` + `PowerBuffer` still open. Story 1.5 closed the two production bugs Story 1.4 deferred (PAUSE-unreachable + status-line None-crash). Story 1.6 gated the pre-clamp pause on the `can_switch` hysteresis (closes the asymmetric-flapping risk surfaced by Story 1.5 CR). | 1-2 days |
